...
public class PicassoUtlis {
    ...
    public static void img(String imageUrl, ImageView imageView) {
        ...
        Picasso.with(context).load(imageUrl)
                .fit()          //这两行是限制加载方式
                .centerCrop()   //类似ImageView的centercrop
                .memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE)//解决图片过大
                .config(Bitmap.Config.RGB_565)//解决图片过大
                .placeholder(R.mipmap.error_pic_s).error(R.mipmap.error_pic_s).into(imageView);
    }
    ...
    public static void img(String imageUrl, ImageView imageView, int placeholder, int with, int height) {
        ...
        Picasso.with(context).load(imageUrl)
                ...
                .memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE)//解决图片过大
                .config(Bitmap.Config.RGB_565)//解决图片过大
                .placeholder(placeholder).resize(with, height).into(imageView);
    ...
    public static void Cornersimg(String imageUrl, ImageView imageView, int conner1) {
        ...
        Picasso.with(context).load(imageUrl)
                ...
                .memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE)//解决图片过大
                .config(Bitmap.Config.RGB_565)//解决图片过大
                .transform(new PicassoRoundTransform(conner1)).into(imageView);
    }
    ...
    public static void rountimg(String imageUrl, ImageView imageView) {
        ...
        Picasso.with(context).load(imageUrl)
                ...
                .memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE)//解决图片过大
                .config(Bitmap.Config.RGB_565)//解决图片过大
                .error(R.drawable.vector_drawable_circle).transform(new CircleTransform()).into(imageView);
    }
    ...
    public static void rountimg(String imageUrl, ImageView imageView, int placeholder) {
        ...
        Picasso.with(context).load(imageUrl)
                ...
                .memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE)//解决图片过大
                .config(Bitmap.Config.RGB_565)//解决图片过大
                .placeholder(placeholder).transform(new CircleTransform()).into(imageView);
    ...
    static class CircleTransform implements Transformation {
        ...
        public Bitmap transform(Bitmap source) {
            ...
            BitmapShader shader = new BitmapShader(squaredBitmap,
                    BitmapShader.TileMode.MIRROR, BitmapShader.TileMode.MIRROR);//定义一个渲染器
